...
router.post("/", (req, res) => {
  ...
  if (req.body.recurringPrice < 100 && req.body.paymentMethod === "cash") {
    // here was a bug: wrong sign in the condition "recurringPrice > 100", changed to"less than"
  ...
  if (req.body.billingInterval === "monthly") {
    ...
    if (req.body.billingPeriods < 6) {
      // here was a bug: "req.billingPeriods" does not exist, "billingPeriods" field is in "body" object
    ...
    // here was a bug: A condition "billingPeriods > 3" has no sense here. Changed to "billingPeriods < 3" respectively to the error message
  } else {
